...
| `doctor [--network]` | User asks "is pacekeeper working?", meters are missing from the status line, or after install/upgrade. Prints ✓/⚠/✗ environment checks with fixes; `--network` also tries a live usage fetch. Pass `--transcript <current transcript path>` when you have it, to also validate transcript-format assumptions. |
