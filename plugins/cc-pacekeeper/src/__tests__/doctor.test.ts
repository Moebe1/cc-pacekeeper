...
describe('doctor', () => {
    test('offline run produces every check with a severity', async () => {
        ...
        for (const expected of ['runtime', 'credentials', 'usage cache', 'config', 'context window override', 'auto-compact window', 'model-info cache', 'state dirs', 'hook crashes', 'plugin version']) {
