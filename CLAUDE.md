...
Dev install: `/plugin marketplace add <path to this checkout>` replaces the GitHub `cc-pacekeeper` marketplace and uninstalls the plugin; then `claude plugin install cc-pacekeeper@cc-pacekeeper`. Shims exec the source from the cache dir, so hooks pick up new code on the next event without a restart; the cache dir is keyed by the version string. Re-add `leaflessbranch/cc-pacekeeper` afterwards.
